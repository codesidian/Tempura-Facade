...
<SensorFilter :filters="filters" v-on:add-filter="setFilter($event)"/>
<v-divider/>
    ...
    trigger:function () {
      ...
      var d = new Date();
        for (i = 0; i < this.sensors.length; i++){
          ...
          this.sensors[i].lastUpdated = d.getHours() +':'+d.getMinutes()
        }
      },
      untrigger:function () {
      var i;
    ...
    computed: {
        filteredList() {
            ...
            if(this.filters != ""){
                ...
                sensorlist = this.sensors.filter(sensor => this.filters.includes(sensor.location.toLowerCase()) );
            }
            else{sensorlist =  this.sensors;}
            return sensorlist;
        }
    },
    props:{sensors:[Array],},
